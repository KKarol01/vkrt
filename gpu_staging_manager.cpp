...
//    assert(submit_queue);
//
//    auto binfo = Vks(VkBufferCreateInfo{ .size = pool_size_bytes, .usage = VK_BUFFER_USAGE_TRANSFER_SRC_BIT });
//
...
bool GpuStagingManager::send_to(std::span<const GpuStagingUpload> uploads, VkSemaphore wait_sem, VkSemaphore signal_sem,
    ...
    for(const auto& upload : uploads) {
        ...
        if(transactions.front().dst_type == IMAGE) {
            ...
            assert(false); // fix comment below
    ...
    if(all_uploads_empty) { signal_sems.emplace_back(signal_sem, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT); }
