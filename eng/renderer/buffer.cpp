...
Buffer::Buffer(const std::string& name, VkDevice dev, VmaAllocator vma, size_t alignment,
               const VkBufferCreateInfo& vk_info, const VmaAllocationCreateInfo& vma_info) noexcept
    : name(name), dev(dev), vma(vma), alignment(std::min(alignment, 1ull)), vk_info(Vks(VkBufferCreateInfo{ vk_info })),
      vma_info(vma_info) {
    if(!dev || !vma) { return; }
    ...
    if(alignment > 1ull) {
        VK_CHECK(vmaCreateBufferWithAlignment(vma, &this->vk_info, &this->vma_info, alignment, &buffer, &alloc, &alloc_info));
    } else {
...
Buffer::Buffer(const std::string& name, VkDevice dev, VmaAllocator vma, buffer_resizable_t resizable, size_t alignment,
               const VkBufferCreateInfo& vk_info, const VmaAllocationCreateInfo& vma_info) noexcept
    : Buffer(name, dev, vma, alignment, vk_info, vma_info) {
