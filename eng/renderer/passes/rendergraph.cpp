...
void RenderGraph::bake()
{
    ...
    for(const auto& pass : passes)
    {
        ...
        for(const auto& e : pass->accesses)
        {
            ...
            else { ENG_ERROR("Invalid barrier type"); }
        }
    }
    ...
    for(const auto& [res, hist] : history)
    {
        ...
        else { ENG_ERROR("Invalid resource type"); }
    }
}
    ...
    for(auto& s : stages)
    {
        ...
        for(const auto& p : s.passes)
        {
            for(const auto& a : p->accesses)
            {
                ...
                if(r.is_buffer()) { (bit++)->buffer = unpack_buffer(r.resource_cb()).buffer; }
                else if(r.is_image())
                {
...
{
    return RendererVulkan::get_instance()->get_buffer(std::get<Handle<Buffer>>(handle));
}
...
Image& RenderGraph::unpack_image(resource_ht handle)
{
    ...
    return RendererVulkan::get_instance()->get_image(img);
}
