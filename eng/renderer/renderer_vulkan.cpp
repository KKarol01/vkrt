...
void RendererVulkan::initialize_resources() {
    ...
    staging_buffer = new StagingBuffer{
        submit_queue,
        make_buffer("staging_buffer", buffer_resizable, 1ull,
    ...
    vertex_positions_buffer =
        make_buffer("vertex_positions_buffer", buffer_resizable, 1ull,
    ...
    render_graph = new rendergraph::RenderGraph{};
    ...
    for(uint32_t i = 0; i < frame_datas.size(); ++i) {
        ...
        fd.constants =
            make_buffer(std::format("constants_{}", i), 1ull,
        ...
        fd.transform_buffers =
            make_buffer(std::format("transform_buffer_{}", i), 1ull,
                        Vks(VkBufferCreateInfo{ .size = 512ul, .usage = VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT }),
                        VmaAllocationCreateInfo{});
    }

    vsm.constants_buffer = make_buffer("vms buffer", 1ull,
    ...
    vsm.free_allocs_buffer =
        make_buffer("vms alloc buffer", 1ull,
...
void RendererVulkan::build_render_graph() {
    ...
    render_graph->clear_passes();
    render_graph->add_pass<ZPrepassPass>()
        .add_pass<VsmClearPagesPass>()
        .add_pass<VsmPageAllocPass>()
        .add_pass<VsmShadowPass>()
        //.add_pass<VsmDebugPageAllocCopy>()
        .add_pass<DefaultUnlitPass>()
        .add_pass<ImguiPass>()
        .add_pass<SwapchainPresentPass>()
        .bake();
}
...
void RendererVulkan::render() {
    ...
    pipeline_compiler.threaded_compile();
    bindless_pool->update();
...
void RendererVulkan::bake_indirect_commands() {
    ...
    for(uint32_t i = 0u; i < mesh_instances.size(); ++i) {
        ...
        gpu_mesh_instances.push_back(GPUMeshInstance{
            ...
            .color_texture_idx = get_bindless_index(make_image_view(
                mat.textures.base_color_texture.handle, VK_IMAGE_LAYOUT_READ_ONLY_OPTIMAL,
                samplers.get_sampler(mat.textures.base_color_texture.filter, mat.textures.base_color_texture.addressing))),
            .normal_texture_idx =
                get_bindless_index(make_image_view(mat.textures.normal_texture.handle, VK_IMAGE_LAYOUT_READ_ONLY_OPTIMAL,
                                                   samplers.get_sampler(mat.textures.normal_texture.filter,
                                                                        mat.textures.normal_texture.addressing))),
            .metallic_roughness_idx =
                get_bindless_index(make_image_view(mat.textures.metallic_roughness_texture.handle, VK_IMAGE_LAYOUT_READ_ONLY_OPTIMAL,
    ...
    if(!indirect_draw_buffer){
        indirect_draw_buffer = make_buffer("indirect draw", 1ull,
            Vks(VkBufferCreateInfo{
    ...
    if(!mesh_instances_buffer) {
        mesh_instances_buffer = make_buffer("mesh instances", 1ull,
            Vks(VkBufferCreateInfo{
...
Handle<Buffer> RendererVulkan::make_buffer(const std::string& name, size_t alignment, const VkBufferCreateInfo& vk_info,
                                           const VmaAllocationCreateInfo& vma_info) {
    const auto handle =
        buffers.emplace(Handle<Buffer>{ generate_handle }, Buffer{ name, dev, vma, alignment, vk_info, vma_info })
            .first->first;
    bindless_pool->register_buffer(handle);
    return handle;
}

Handle<Buffer> RendererVulkan::make_buffer(const std::string& name, buffer_resizable_t resizable, size_t alignment,
                                           const VkBufferCreateInfo& vk_info, const VmaAllocationCreateInfo& vma_info) {
    const auto handle =
        buffers
            .emplace(Handle<Buffer>{ generate_handle }, Buffer{ name, dev, vma, resizable, alignment, vk_info, vma_info })
            .first->first;
    bindless_pool->register_buffer(handle);
    return handle;
}
...
Image& RendererVulkan::get_view_image(VkImageView view) { return *views.at(view); }
