...
void VkSwapchainMetadata::init(Swapchain& a)
{
    ...
    if(a.images.empty())
    {
        ENG_ERROR("Swapchain has not been initialized.");
        return;
    }
    ...
    a.(VkDevice dev, uint32_t image_count, uint32_t width, uint32_t height);
}
...
void VkSwapchainMetadata::destroy(Swapchain& a)
{
    if(!a.metadata) { return; }
    auto& md = get(a);
    vkDestroySwapchainKHR(RendererVulkan::get_instance()->dev, md.swapchain, nullptr);
    delete &md;
    md = nullptr;
    a.images.clear();
    a.views.clear();
...
Swapchain::Swapchain(uint32_t num_images)
{
...
Handle<Image> Swapchain::get_current_image() { return images.at(current_index); }
